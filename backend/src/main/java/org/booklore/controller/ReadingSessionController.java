...
public class ReadingSessionController {
    ...
    @Operation(summary = "Record multiple reading sessions", description = "Receive offline-queued reading sessions from KOReader and persist them.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Reading sessions recorded successfully"),
            @ApiResponse(responseCode = "400", description = "Invalid payload")
    })
    @PostMapping("/batch")
    public ResponseEntity<ReadingSessionBatchResponse> recordBatchSessions(@RequestBody @Valid ReadingSessionBatchRequest request) {
